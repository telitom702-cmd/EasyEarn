...
const path = require('path'); // index.html সার্ভ করার জন্য যোগ করা হয়েছে
...
// --- Frontend (index.html) সার্ভ করার জন্য অত্যন্ত গুরুত্বপূর্ণ ---
app.use(express.static(path.join(__dirname, '/')));

// --- MongoDB Connection (Optimized) ---
const connectDB = async () => {
    if (mongoose.connection.readyState >= 1) return; // যদি আগেই কানেক্টেড থাকে তবে আর কানেক্ট হবে না
    try {
        await mongoose.connect(config.MONGO_URI, {
            dbName: "telitask" // ডাটাবেসের নাম দেওয়া হলো
        });
...
connectDB(); // সার্ভার স্টার্ট হওয়ামাত্রই ডাটাবেস কানেক্ট হবে
...
app.post('/api/register', async (req, res) => {
    const { uid, name, deviceId, vpnInfo, settings } = req.body;
    if (!uid) return res.status(400).json({ error: "Telegram UID required" });
    try {
        let user = await User.findOne({ uid });
        if (!user) { user = new User({ uid, name, deviceId, vpnInfo, settings }); } 
        else {
            if (name) user.name = name; if (deviceId) user.deviceId = deviceId;
...
app.get('/api/user/:uid', async (req, res) => { const user = await User.findOne({ uid: req.params.uid }); res.json(user); });
...
app.post('/api/complete-task', async (req, res) => {
    ...
    if (user.completedTasks && user.completedTasks.includes(taskId)) return res.json({ success: false, message: 'আপনি এই টাস্কটি আগেই করেছেন!' });
    user.balance += Number(reward); user.completedTasks.push(taskId);
    await user.save(); res.json({ success: true, message: 'টাকা যোগ হয়েছে!' });
});
...
// =======================================================
// UNIVERSAL SETUP: Local + Vercel + Render Support
// =======================================================

// Vercel-এর জন্য সরাসরি Express অ্যাপটি এক্সপোর্ট করা হচ্ছে
module.exports = app;

// Render এবং Local কম্পিউটারে রান করার জন্য
